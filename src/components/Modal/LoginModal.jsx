...
class LoginModal extends React.Component {
  ...
  handleSubmit(event) {
    ...
    if (form.checkValidity() === false) {
      ...
      window.cache.WebApi.tokens()
        ...
        .catch((error) => {
          showError(
            error.response.status === 401 ? "Credentials failed" : error.message
          );
        });
    }
  }
  ...
  render() {
    return (
      <BaseModal
        ref={this.baseModal}
        titleTextId="loginTitle"
        subtitleTextId="loginSubtitle"
        dismissButtonTextId="exit"
        confirmButtonTextId="loginTitle"
